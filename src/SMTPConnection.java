...
public class SMTPConnection {
	...
	public SMTPConnection(Envelope envelope) throws IOException {
		...
		try {
			sendCommand("HELO " + localhost, 250); /* Command: HELO Port: 250  */
			System.out.println("Send command : HELO, Port: 250");
		} catch (IOException e) {
	...
	public void send(Envelope envelope) throws IOException {
		sendCommand("MAIL FROM:<" + envelope.Sender + ">", 250); /* Command: MAIL FROM Port: 250  */
		System.out.println("Send command : MAIL FROM, Port: 250");
		sendCommand("RCPT TO:<" + envelope.Recipient + ">", 250); /* Command: RCPT TO Port: 250  */
		System.out.println("Send command : RCPT TO, Port: 250");
		sendCommand("DATA", 354); /* Command: DATA Port: 354  */
		System.out.println("Send command : DATA, Port: 354");
		sendCommand(envelope.Message.toString() + CRLF + ".", 250); /* send a dot to recognize that data is finish  */
		System.out.println("Send command : ., Port: 250");
	}
	...
	public void close() {
		isConnected = false;
		try {
			sendCommand("QUIT", 221); /* Command: QUIT Port: 221  */
			connection.close();
		} catch (IOException e) {
